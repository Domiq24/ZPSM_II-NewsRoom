...
class NewsService {
    ...
    public async insertSavedNews(userID: number, newsID: number) {
        try {
            const result = await this.client.query(
                "INSERT INTO news_room.saved_news(user_id, news_id) " +
    ...
    public async insertOrUpdateRating(userID: number, newsID: number, value: number) {
        try {
            ...
            let result;
            if (rating.rows[0].count < 1) {
                result = await this.client.query(
                    "INSERT INTO news_room.ratings(user_id, news_id, value) " +
                    `VALUES (${userID}, ${newsID}, ${value})`
                );
            } else {
                result = await this.client.query(
                    "UPDATE news_room.ratings " +
                    `SET value=${value} ` +
            ...
            console.error("Error while inserting/updating rating: ", error);
            throw new Error("Error while inserting/updating rating");
        }
    }
